...
export function TeamCard({
  ...
  const s = computeStats(team.members || []);
  ...
  return (
    <Card className="flex flex-col gap-4 p-5">
      <div className="flex items-start justify-between gap-3">
        ...
        {s.valid ? (
          <GlowingBadge variant="success">Valid · {s.memberCount}/2</GlowingBadge>
        ) : (
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
        <span>👥 {s.memberCount} / 2 members</span>
        <span>🏛 {s.sameDept ? "Same Dept" : "Mixed Depts"}</span>
        <span>⚡ {s.differentSkills ? "Diverse Skills" : "Overlapping Skills"}</span>
      </div>
